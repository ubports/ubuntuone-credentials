...
void TestHeader::testSetAlbum()
{
    ...
    header.setAlbum(album);

    QCOMPARE(header.ui->lblAlbum->text(), album);
}
...
void TestHeader::testSetArtist()
{
    ...
    QCOMPARE(header.ui->lblArtist->text(), artist);
}
...
void TestHeader::testSetPrice()
{
    ...
    QCOMPARE(header.ui->lblPrice->text(), price);
}

void TestHeader::testSetPicture()
{
    QVERIFY(header.ui->lblImage->pixmap() == NULL);
    QString picture = "/home/path/picture.png";
    header.setPicture(picture);
